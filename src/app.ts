...
function readGatewayToken(): string | null {
  ...
  try {
    const raw = fs.readFileSync('/home/leogray/.openclaw/openclaw.json', 'utf8');
...
export function createApp() {
  ...
  app.post('/pair/handoff/redeem', (req, res) => {
    ...
    h.used = true;
    res.json({ ok: true, dashboardUrl: 'https://uben-claw.lan/', gatewayToken: token, note: 'One-time handoff redeemed. Paste token in Control UI settings.' });
  });
